...
public class ServerImpl extends SiloServiceGrpc.SiloServiceImplBase {
	...
	public void track(TrackRequest request, StreamObserver<TrackResponse> responseObserver) {
		...
		try {
			response = TrackResponse.newBuilder().setObservation(transform(this.op.track(request.getType(), request.getId()))).build();
			responseObserver.onNext(response);
		    responseObserver.onCompleted();		
	...
	public void camInfo(CamInfoRequest request, StreamObserver<CamInfoResponse> responseObserver) {
		...
		try {
			response = CamInfoResponse.newBuilder().setCoordinates(this.transformCoord((op.cam_info(request.getName())))).build();
			responseObserver.onNext(response);
			responseObserver.onCompleted();
		} catch (NoSuchCameraException e) {
			responseObserver.onError(Status.INVALID_ARGUMENT.withDescription("No camera found with given name!").asRuntimeException());
